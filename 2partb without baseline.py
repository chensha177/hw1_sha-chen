...
class PGNetwork:
  def __init__(self,state_size,action_size,learning_rate,name="PGNetwork"):
    ...
    with tf.variable_scope("name"):
      ...
      #   self.conv2=tf.layers.conv2d(inputs=self.conv1_out,filters=16,kernel_size=[4,4],strides=[2,2],padding="SAME",kernel_initializer=tf.initializers.glorot_uniform(),name="conv2")
      ...
      #   self.conv3=tf.layers.conv2d(inputs=self.conv2_out,filters=32,kernel_size=[3,3],strides=[1,1],padding="VALID",kernel_initializer=tf.contrib.layers.xavier_initializer_conv2d(),name="conv3")
      ...
      with tf.name_scope("flatten"):
        self.flatten= tf.layers.flatten(self.conv1)
      with tf.name_scope("fc1"):
        self.fc=tf.layers.dense(inputs=self.flatten,units=64,activation=tf.nn.relu,kernel_initializer=tf.initializers.glorot_uniform(),name="fc1")
      
      #with tf.name_scope("logits"):
        self.logits=tf.layers.dense(inputs=self.fc,kernel_initializer=tf.initializers.glorot_uniform(),units=action_size,activation=None)
      with tf.name_scope("softmax"):
        self.action_distribution=tf.nn.softmax(self.logits)
      # with tf.name_scope("fc2"):
      #   self.val=tf.layers.dense(inputs=self.flatten,units=512,activation=tf.nn.relu,kernel_initializer=tf.contrib.layers.xavier_initializer(),name="fc2")
...
def make_batch(batch_size):
  ...
  while True:
    ...
    action=np.random.choice(range(action_size),p=action_probability_distribution[0])
    states.append(state)
...
if training:
  while epoch<num_epochs+1:
   ...
   # print(rewards_of_batch)
    ...
    sess.run([PGNetwork.train_opt],feed_dict={PGNetwork.inputs_: states_mb,PGNetwork.actions: actions_mb,PGNetwork.discounted_episode_rewards_: discounted_rewards_mb})
